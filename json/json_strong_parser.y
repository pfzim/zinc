...
elements
	:    svalue						{ $$.flags = JS_ARRAY; $$.arr_val = NULL; json_array_push(&$$, json_dup0(&$1)); }
	|    elements ',' svalue		{ $$ = $1; json_array_push(&$$, json_dup0(&$3)); }
;
...
type
	:    T_LABEL					{
										if(stricmp($1.str_val, "ulong") == 0)
										{
											$$.flags = JS_ULONG; 
										}
										else if(stricmp($1.str_val, "string") == 0)
										{
											$$.flags = JS_STRING; 
										}
										else if(stricmp($1.str_val, "long") == 0)
										{
											$$.flags = JS_LONG; 
										}
										else if(stricmp($1.str_val, "double") == 0)
										{
											$$.flags = JS_DOUBLE; 
										}
										else if(stricmp($1.str_val, "bool") == 0)
										{
											$$.flags = JS_ULONG; 
										}
										free_str($1.str_val);
