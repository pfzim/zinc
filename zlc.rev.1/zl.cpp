	...
	while(instr(zl_eip))
	{
		...
		switch(instr(zl_eip))
		{
			...
			case OP_PUSH_MEM:
				nextinstr(zl_eip);
				push(zl_esp);
				pdw(zl_esp) = mem(zl_eip);
				zl_eip += 4;
				continue;
			case OP_PUSH_PMEM:
				nextinstr(zl_eip);
				push(zl_esp);
				pdw(zl_esp) = pmem(zl_eip);
				zl_eip += 4;
				continue;
			case OP_PUSH_OFFSET:
				nextinstr(zl_eip);
				push(zl_esp);
				pdw(zl_esp) = dw(datatable + pdw(zl_eip));
				zl_eip += 4;
				continue;
...
{
	char *regs[] = {"eip", "esp", "eax", "ecx", "ebx", "edx", "esd", "ebd", "esi", "edi", "eflags", "error"};
